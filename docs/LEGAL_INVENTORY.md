...
8 September delivery update: the [About/license audit](ABOUT_LICENSE_AUDIT.md)
records corrected product packaging, offline notices, and the current dependency
inventory. It does not close the outstanding ownership or full-development
distribution reviews below.
...
| --- | --- | --- | --- | --- |
| FP-001 | Rust server | Root [`AGPL-3.0-only`](../LICENSE); prior MIT text preserved at [`apps/server/LICENSE-MIT-HISTORICAL`](../apps/server/LICENSE-MIT-HISTORICAL) | `CLEAR` for current first-party code | Maintain the private ownership register and preserve permissions already granted for historical MIT versions |
...
| FP-007 | Original AI-assisted English-to-Spanish and Spanish-to-English curriculum | Owner-approved `AGPL-3.0-only` scope in [LICENSING.md](LICENSING.md#first-party-curriculum); [9 September approval receipt](CURRICULUM_LICENSE_APPROVAL_20260909.json) records exact source hashes and prior metadata | `CLEAR` for the identified first-party curriculum | Preserve third-party terms and pending native-speaker/pronunciation reviews; licensing does not activate a development course |
| FP-008 | Original AI-assisted Norwegian Bokmål curriculum and original geometric Norwegian flag assets | [Owner license approval](CURRICULUM_LICENSE_APPROVAL_20260909.json) grants `AGPL-3.0-only` for exact sources/assets; the earlier [AI review receipt](../apps/languages/norwegian-bokmal/content/reviews/onboarding-20260909.json) records grammar evidence without copied dictionary definitions/examples | `CLEAR` for the identified first-party curriculum and flag assets | Preserve the historical review receipt, source attribution and unchanged native-review status; do not extend this clearance to third-party material |
...
| SW-002 | Transformers.js | 4.2.0 | Shared tracked bundle and Apache-2.0 text at [`apps/language-runtime/vendor/transformers`](../apps/language-runtime/vendor/transformers/); exact bundle hash is pinned by [`embedding-runtimes.json`](../apps/language-runtime/embedding-runtimes.json) | `CLEAR-WITH-NOTICE` | Keep the pinned Apache-2.0 text in every browser distribution that serves the bundle |
...
| SW-004 | ONNX Runtime Web | `1.26.0-dev.20260416-b7804b056c` | Revision recorded in the embedding runtime manifest; supplemental [upstream MIT text](../apps/language-runtime/static/legal/ONNX-RUNTIME-LICENSE.txt) now registered for offline delivery | `CLEAR-WITH-NOTICE` for corrected source; existing APK 170 lacks this text | Preserve the offline text and validate the next release's packaged/downloaded union |
| SW-005 | Rust and Android dependencies | Locked dependency graphs exist | [Product Android inventory](../apps/language-runtime/static/legal/android-dependencies.json) and offline notices generated from resolved POMs; other distributions remain unaudited | `REVIEW` outside audited Android product scope | Maintain the product Gradle notice guard; separately inventory Rust and full-development Android dependencies |
| SW-006 | Archived Three.js / GLTFLoader experiment and centralized Quaternius preview motion package | Archived browser code remains under [`archive/demos`](../archive/demos/); the active GLB, CC0 notice, hashes, and outstanding review requirements are recorded in its [strict manifest](../apps/launcher/static/assets/motion/quaternius-standard-v1/manifest.json) | `NOT-DISTRIBUTED`; Caatuu Game remains local-preview-only | Complete the package manifest's source, retrieval, modification, and license review requirements before distributing Caatuu Game; keep archived browser dependencies outside active routes |
