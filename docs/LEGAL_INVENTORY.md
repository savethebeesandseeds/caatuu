...
| SW-006 | Archived Three.js / GLTFLoader experiment and centralized Quaternius preview motion package | Archived browser code remains under [`archive/demos`](../archive/demos/); the active GLB, CC0 notice, hashes, and outstanding review requirements are recorded in its [strict manifest](../apps/launcher/static/assets/motion/quaternius-standard-v1/manifest.json) | `NOT-DISTRIBUTED`; Memory Moon remains local-preview-only | Complete the package manifest's source, retrieval, modification, and license review requirements before distributing Memory Moon; keep archived browser dependencies outside active routes |
