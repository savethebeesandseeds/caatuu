...
| --- | --- | --- |
| GitHub Pages | Serves the launcher, active Czech product and manifest-enabled `noindex` development previews, embedded course games, ordinary setup assets, and retained Android downloads; it does not publish the standalone Godot preview | `.github/workflows/pages.yml` and `apps/launcher/tooling/build-pages-site.mjs` |
...
catalog course with `platforms.browser.pagesEnabled`: Czech and the `noindex`
Mandarin, Spanish, English-from-Spanish and Norwegian Bokmål development
previews. It overlays every release in the append-only Android descriptor on
the fixed 162/161 preservation archive. The
[9 September owner approval](CURRICULUM_LICENSE_APPROVAL_20260909.json) clears
the identified original Spanish-course and Norwegian curricula for distribution;
their native-speaker and pronunciation reviews remain pending. Android-only
publication preserves the deployed website snapshot; registering a route does
not itself publish that course's website.
The newest descriptor release becomes the stable aliases, while every older
immutable route remains. The historical v163 snapshot retained these
