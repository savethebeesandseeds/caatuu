...
- Perform all authorized repository work directly in the canonical `C:\Work\caatuu` checkout on `main`. Before any file write, commit, publication, or other mutation, verify all of the following: `git branch --show-current` returns exactly `main`; `refs/heads` contains only `refs/heads/main`; no non-`main` remote-tracking branch exists (a remote's symbolic `HEAD` is not a branch); and this task is using only the canonical checkout. If any check fails, stop before modifying the project, report the exact mismatch and branch names to the user, and wait for explicit consolidation direction.
- Concurrent sessions and existing modified, staged, or untracked files in the canonical checkout are expected and do not by themselves block work or require user confirmation. Preserve other sessions' changes and coordinate overlapping edits. A registered detached worktree alone does not establish active use and does not block work in the canonical checkout. If there is concrete evidence that another checkout is actively being used for Caatuu implementation, serving, or validation, stop before modifying the project, report the exact checkout and evidence, and wait for explicit coordination direction. Do not alter or remove that checkout as part of this check.
