...
file; `compose.archive-openai.yaml` mounts only that file as
`OPENAI_API_KEY_FILE=/run/secrets/openai-api-key`.

The launch environment no longer sources `apps/caatuu-runtime/env.local.sh`.
...
Add `-f compose.archive-openai.yaml` only when the secret file exists and those
...
docker exec caatuu-dev bash -lc `
  'cd /workspace && node tools/runtime/refresh-setup-assets.mjs --check'
```
