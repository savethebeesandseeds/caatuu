...
function auditRepoOwnership() {
  ...
  assert(
    start.includes('runtime_dir="$workspace_root/apps/caatuu-runtime"') && start.includes('cd "$runtime_dir"'),
    "runtime start script should cd into apps/caatuu-runtime"
  );
...
function auditApk() {
  ...
  const forbiddenEntryPatterns = [
    ...
    /^assets\/assets\/icons\/(?:china_flag|czech_flag|dark_mode|english_flag|france_flag|germany_flag|japan_flag|spain_flag)\.png$/,
  ];
...
function auditSetupManifest() {
  ...
  for (const artifact of artifacts) {
    ...
    const sourcePath = decodedUrl.startsWith("/assets/")
      ? join(workspaceRoot, "apps/caatuu-unified/static", decodedUrl.slice(1))
      : decodedUrl.startsWith("/cz/")
        ? join(workspaceRoot, "apps/caatuu-czech/static", decodedUrl.slice(4))
        : "";
...
function auditAndroidSource() {
  ...
  const nativeModelPath = join(workspaceRoot, "apps/caatuu-android/app/src/main/java/com/caatuu/android/NativeCzechModel.kt");
  ...
  const dictionaryManagerPath = join(workspaceRoot, "apps/caatuu-android/app/src/main/java/com/caatuu/android/DictionaryManager.kt");
  ...
  const bridgePath = join(workspaceRoot, "apps/caatuu-android/app/src/main/java/com/caatuu/android/CaatuuBridge.kt");
  const appUpdateManagerPath = join(workspaceRoot, "apps/caatuu-android/app/src/main/java/com/caatuu/android/AppUpdateManager.kt");
  const filePathsPath = join(workspaceRoot, "apps/caatuu-android/app/src/main/res/xml/caatuu_file_paths.xml");
  ...
  const runtimeMainPath = join(workspaceRoot, "apps/caatuu-runtime/src/main.rs");
  const runtimeRoutesPath = join(workspaceRoot, "apps/caatuu-runtime/src/routes/mod.rs");
  const composePath = join(workspaceRoot, "compose.yaml");
  const phoneDebugComposePath = join(workspaceRoot, "compose.phone-debug.yaml");
  const termuxInstallPath = join(workspaceRoot, "tools/android-build/termux-install-debug.sh");
  const androidVersionsPath = join(workspaceRoot, "tools/android-build/versions.env");
  const setupJdkPath = join(workspaceRoot, "tools/android-build/setup-jdk.sh");
  const prepareLlamaPath = join(workspaceRoot, "apps/caatuu-android/scripts/prepare-llama-vendor.sh");
  const prepareLlamaPowerShellPath = join(workspaceRoot, "apps/caatuu-android/scripts/prepare-llama-vendor.ps1");
  const llamaPatchPath = join(workspaceRoot, "apps/caatuu-android/patches/llama-android-thinking.patch");
  ...
  assert(prepareLlama.includes('source "$repo_root/tools/android-build/versions.env"'), "llama.cpp preparation should load the pinned Android versions");
