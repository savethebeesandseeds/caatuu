...
const czechStatic = new URL("../../../apps/caatuu-czech/static/", import.meta.url);
const unifiedStatic = new URL("../../../apps/caatuu-unified/static/", import.meta.url);
const androidBuild = new URL("../../../apps/caatuu-android/app/build.gradle.kts", import.meta.url);

const [chromeCss, appCss, chatCss, chromeJs, courseProfile, serviceWorker, unifiedCss, unifiedHtml, unifiedJs, languageRegistry, androidGradle] = await Promise.all([
  ...
  readFile(new URL("app.css", unifiedStatic), "utf8"),
  readFile(new URL("index.html", unifiedStatic), "utf8"),
...
test("the shared Czech header uses the configured PNG without a CSS frame", () => {
  ...
  assert.match(courseProfile, /flagClass: "cz-flag",\s*flagSrc: "logos\/czech_flag\.png"/, "the Czech profile should select its PNG flag");
  assert.match(serviceWorker, /"\.\/logos\/czech_flag\.png"/, "the Czech flag must remain available offline");
  assert.doesNotMatch(androidGradle, /exclude\("logos\/czech_flag\.png"\)/, "the Android package must include the Czech flag");
});

test("the language landing page uses the registered Czech PNG without a frame", () => {
  assertBorderlessImage(unifiedCss, ".flag-icon");
  const holder = ruleBody(unifiedCss, ".language-list li");
  ...
  assert.match(unifiedHtml, /<img class="flag-icon" src="\/assets\/icons\/czech_flag\.png" alt="">/);
  assert.match(unifiedHtml, /<span class="language-choice-code">CZ<\/span>/, "the fallback language row should name Czech explicitly");
  ...
  assert.match(unifiedJs, /code\.textContent = language\.shortCode/, "dynamic language rows should include their short code");
});
