...
async function fixture() {
  ...
  const unifiedStaticDir = join(workspaceRoot, "apps/caatuu-unified/static");
  const languageStaticDir = join(workspaceRoot, "apps/caatuu-czech/static");
  await mkdir(join(unifiedStaticDir, "assets/images"), { recursive: true });
  await mkdir(join(languageStaticDir, "data"), { recursive: true });
  await writeFile(join(unifiedStaticDir, "assets/images/example.png"), "shared-image");
  ...
  return { workspaceRoot, unifiedStaticDir, languageStaticDir, manifestPath };
}
...
test("check mode detects drift without changing the manifest", async (t) => {
  ...
  await writeFile(join(paths.unifiedStaticDir, "assets/images/example.png"), "corrected-image");
