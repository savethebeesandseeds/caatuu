...
import { ENGLISH_AMERICAN_CONTENT_POLICY_ID } from "../policies/english-american.mjs";
import { defineWordWorldProjectionPolicy } from "./contract.mjs";

export const ENGLISH_AMERICAN_WORD_WORLD_PROJECTION_POLICY_ID =
  "english-american-word-world-v1";

export const ENGLISH_AMERICAN_WORD_WORLD_PATHS = Object.freeze({
  learnerBaseSource: "apps/languages/shared/learner-base-realizations/es-ES/word-world-starter-v1.json",
  learnerBaseRuntime: "apps/languages/english-from-spanish/static/data/games/word-world/learner-base.json",
  conceptsSource: "apps/languages/shared/english-concepts/word-world-es-en-v1.json",
  realizationsSource: "apps/languages/english-from-spanish/content/word-world/starter-v1.realizations.json",
  conceptsRuntime: "apps/language-runtime/static/data/english-concepts/word-world-es-en-v1.json",
  realizationsRuntime: "apps/languages/english-from-spanish/static/data/games/word-world/content.json",
  manifest: "apps/languages/english-from-spanish/static/data/games/word-world/manifest.json"
});

export const englishAmericanWordWorldProjectionPolicy = defineWordWorldProjectionPolicy({
  id: ENGLISH_AMERICAN_WORD_WORLD_PROJECTION_POLICY_ID,
  contentPolicyId: ENGLISH_AMERICAN_CONTENT_POLICY_ID,
  defaultPaths: ENGLISH_AMERICAN_WORD_WORLD_PATHS,
  supplementalOutputs: {},
  manifestBindings: {
    englishProjection: {
  ...
  targetProjectionPolicy() {
    return {
      ...
      reason: "English pronunciation is omitted until a separate reviewed pronunciation policy exists."
    };
  },
  projectSupplemental() {
    return {};
  ...
  validate({ runtimeManifest, targetProjection, realizations, paths }) {
    ...
    if (!isDeepStrictEqual(targetProjection.license, realizations.license)) {
      throw new Error("English target projection license must preserve target-content authority.");
    }
    if (Object.hasOwn(runtimeManifest, "targetTextGuide")) {
      throw new Error("English Word World must not publish an unowned target-text guide.");
    }
  }
});
...
function validateManifestProjection(manifest, realizations, paths) {
  ...
  if (manifest.courseId !== realizations.courseId) {
    throw new Error("English Word World manifest courseId does not match the realization catalog.");
  }
  if (
    manifest.sourceConceptCatalog !== expectedConceptCatalog
    || manifest.realizationFile !== expectedRealizationFile
  ) {
    throw new Error("English Word World manifest files do not match the generated runtime projections.");
  }
  if (manifest.targetLanguage !== realizations.targetLanguage.languageTag) {
    throw new Error("English Word World manifest targetLanguage does not match the realization catalog.");
  }
  if (
    ...
    throw new Error("English Word World manifest review gate differs from its target authority.");
  }
  if (manifest.recordCount !== realizations.realizations.length) {
    throw new Error("English Word World manifest recordCount does not match the realization catalog.");
  }
  if (!isDeepStrictEqual(manifest.license, realizations.license)) {
    throw new Error("English Word World manifest license must preserve target-content authority.");
  }
}
...
export default englishAmericanWordWorldProjectionPolicy;
