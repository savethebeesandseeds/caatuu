...
import { NORWEGIAN_BOKMAL_CONTENT_POLICY_ID } from "../policies/norwegian-bokmal.mjs";
import { defineWordWorldProjectionPolicy } from "./contract.mjs";

export const NORWEGIAN_BOKMAL_WORD_WORLD_PROJECTION_POLICY_ID =
  "norwegian-bokmal-word-world-v1";

export const NORWEGIAN_BOKMAL_WORD_WORLD_PATHS = Object.freeze({
  conceptsSource: "apps/languages/shared/english-concepts/word-world-nb-v1.json",
  realizationsSource: "apps/languages/norwegian-bokmal/content/word-world/starter-v1.realizations.json",
  conceptsRuntime: "apps/language-runtime/static/data/english-concepts/word-world-nb-v1.json",
  realizationsRuntime: "apps/languages/norwegian-bokmal/static/data/games/word-world/content.json",
  manifest: "apps/languages/norwegian-bokmal/static/data/games/word-world/manifest.json"
});

export const norwegianBokmalWordWorldProjectionPolicy = defineWordWorldProjectionPolicy({
  id: NORWEGIAN_BOKMAL_WORD_WORLD_PROJECTION_POLICY_ID,
  contentPolicyId: NORWEGIAN_BOKMAL_CONTENT_POLICY_ID,
  defaultPaths: NORWEGIAN_BOKMAL_WORD_WORLD_PATHS,
  supplementalOutputs: {},
  manifestBindings: {
    englishProjection: {
  ...
  targetProjectionPolicy() {
    return {
      ...
      reason: "Norwegian Bokmål pronunciation is omitted until a separate reviewed pronunciation policy exists."
    };
  },
  projectSupplemental() {
    return {};
  ...
  validate({ runtimeManifest, targetProjection, realizations, paths }) {
    ...
    if (!isDeepStrictEqual(targetProjection.license, realizations.license)) {
      throw new Error("Norwegian Bokmål target projection license must preserve target-content authority.");
    }
    if (Object.hasOwn(runtimeManifest, "targetTextGuide")) {
      throw new Error("Norwegian Bokmål Word World must not publish an unowned target-text guide.");
    }
  }
});
...
function validateManifestProjection(manifest, realizations, paths) {
  ...
  if (manifest.courseId !== realizations.courseId) {
    throw new Error("Norwegian Bokmål Word World manifest courseId does not match the realization catalog.");
  }
  if (
    manifest.sourceConceptCatalog !== expectedConceptCatalog
    || manifest.realizationFile !== expectedRealizationFile
  ) {
    throw new Error("Norwegian Bokmål Word World manifest files do not match the generated runtime projections.");
  }
  if (manifest.targetLanguage !== realizations.targetLanguage.languageTag) {
    throw new Error("Norwegian Bokmål Word World manifest targetLanguage does not match the realization catalog.");
  }
  if (
    ...
    throw new Error("Norwegian Bokmål Word World manifest review gate differs from its target authority.");
  }
  if (manifest.recordCount !== realizations.realizations.length) {
    throw new Error("Norwegian Bokmål Word World manifest recordCount does not match the realization catalog.");
  }
  if (!isDeepStrictEqual(manifest.license, realizations.license)) {
    throw new Error("Norwegian Bokmål Word World manifest license must preserve target-content authority.");
  }
}
...
export default norwegianBokmalWordWorldProjectionPolicy;
