...
RUN_ID="${RUN_ID:-qwen3-1.7b-translation-cs-en-001-full-data}"
...
if [ ! -f "$DATASET_DIR/train_all.jsonl" ]; then
  echo "Missing full-data training file: $DATASET_DIR/train_all.jsonl" >&2
  exit 1
fi

if [ ! -d "$INIT_ADAPTER" ]; then
  echo "Missing initial adapter: $INIT_ADAPTER" >&2
  exit 1
fi
