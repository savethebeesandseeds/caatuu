...
Runs the configured phone/app publish path:
  1. Resolve tools/on-device-models/model-configs.json.
...
static_manifest="$TOOL_DIR/../../apps/languages/czech/static/data/models/phone-bench/$MODEL_KEY.manifest.json"
...
Next manual app steps, when this is a new model key:
  - Add the model to apps/android/.../ModelManager.kt.
  - Add selector/legal entries in apps/languages/czech/static/*.html and *.js.
  - Bump the APK version and web cache query strings.
  - Build the APK with apps/android/tooling/build-debug-apk.sh.
