...
`apps/caatuu-czech/static/setup-assets.json` from the authoritative shared and
...
```bash
node tools/runtime/refresh-setup-assets.mjs --check
```
...
```bash
docker compose -f compose.tools.yaml --profile dev up -d caatuu-dev
```

Before running it, increment `versionCode` and `versionName` in
`apps/caatuu-android/app/build.gradle.kts`. The helper refuses to overwrite an
...
docker exec -e ALLOW_SAME_VERSION=1 caatuu-dev bash -lc \
  'cd /workspace && bash tools/android-build/publish-public-debug.sh'
```
...
```powershell
node tools\runtime\audit-runtime-boundary.mjs --base-url http://<your-pc-lan-ip>:8765 --apk artifacts\android\caatuu-debug.apk --allow-debug-artifacts
```
