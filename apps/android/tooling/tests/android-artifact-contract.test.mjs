...
test("native managers apply the shared storage contract before file and recursive-delete operations", async () => {
  ...
  assert.match(vectorManager, /vectorCatalog\.models\.filterNot \{ "vector-dbs\/\$\{it\.fileName\}" in preserveStoragePaths \}\.forEach \{ spec ->[\s\S]*databaseFile\(spec, databasesDir\)[\s\S]*markerFile\(spec, databasesDir\)/u);
  ...
  assert.doesNotMatch(assetClient, /activeCourseId/u);
  assert.match(assetClient, /val requestedCourseId = requestedCourse\?\.id \?: courseRegistry\.defaultCourseId/u);
  assert.match(assetClient, /vectorDatabaseManagers\[requestedCourseId\]/u);
  assert.match(assetClient, /CourseInstallAccess\.assetPath\(resolution, courseSetupReady\)/u);
  assert.match(assetClient, /val assetOwners = staticAssetManagers\.filterValues \{ it\.ownsAssetPath\(assetPath\) \}/u);
  assert.match(assetClient, /courseSetupReady\.invoke\(courseId, false\)/u);
  ...
  assert.match(assetClient, /if \(assetOwners\.isNotEmpty\(\)\) return notFound\(\)/u);
  assert.match(staticAssetManager, /staticAssetStoragePath\(spec\.assetPath, spec\.sha256\)/u);
  assert.match(staticAssetManager, /fileVerifier\.matches\(file, spec\.bytes, spec\.sha256\)/u);
});
