...
if [[ ! -x "$adb_bin" ]]; then
  echo "adb not found at $adb_bin. Run apps/android/tooling/setup-sdk.sh first." >&2
  exit 1
fi

if [[ ! -f "$apk_path" ]]; then
  echo "APK not found at $apk_path. Run apps/android/tooling/build-debug-apk.sh first." >&2
  exit 1
fi
