...
pub fn build_expected_reference_answer(spec: &CorePlusSpec) -> String {
    ...
    format!(
        "{}。{}",
        trim_sentence_trailing_punct(&s1),
        trim_sentence_trailing_punct(&s2)
    )
}
...
pub fn sample_core_plus_core_spec(
    difficulty: &str,
    max_tries: usize,
) -> Result<CorePlusSpec, String> {
    ...
    for _ in 0..max_tries {
        ...
        let step1_pat = step1_pool
            .choose(&mut rng)
            .copied()
            .unwrap_or(step1_pool[0]);
        let step2_pat = step2_pool
            .choose(&mut rng)
            .copied()
            .unwrap_or(step2_pool[0]);
        ...
        let scene = scene_pool
            .choose(&mut rng)
            .copied()
            .unwrap_or(scene_pool[0]);
...
pub fn evaluate_core_plus_core_answer(
    spec: &CorePlusSpec,
    user_answer: &str,
) -> (bool, f32, String) {
    ...
    if answer.is_empty() {
        return (
            false,
            0.0,
            "Empty answer. Write at least one sentence tied to the seed idea.".into(),
        );
    }

    // Intentionally relaxed: this path should reward intent and creativity over rigid form.
    let mut score = 68.0_f32;
    let mut notes: Vec<String> = vec![];

    let (s1, s2, two_sentence_mode) = match split_two_sentences(answer) {
        Some((a, b)) => {
            score += 12.0;
            (a, b, true)
        }
        None => {
            score -= 6.0;
            notes.push("Two sentences are preferred, but one sentence is acceptable.".into());
            let fallback = answer.to_string();
            (fallback.clone(), fallback, false)
        }
    };

    if validate_sentence_pattern_only(&spec.step1, &s1).is_ok() {
        score += 6.0;
    } else {
        score -= 7.0;
        notes.push("Step 1 connector could be clearer.".into());
    }
    if validate_sentence_pattern_only(&spec.step2, &s2).is_ok() {
        score += 6.0;
    } else {
        score -= 7.0;
        notes.push("Step 2 connector could be clearer.".into());
    }

    let seed_phrase = trim_sentence_trailing_punct(&spec.seed);
    if !seed_phrase.is_empty() {
        if answer.contains(&seed_phrase) {
            score += 8.0;
        } else {
            let mut shared = 0usize;
            for ch in seed_phrase.chars() {
                if crate::util::is_cjk(ch) && answer.contains(ch) {
                    shared += 1;
                }
                if shared >= 2 {
                    break;
            ...
            if shared >= 2 {
                ...
                score -= 10.0;
                notes.push("Tie the sentence a bit more clearly to the seed topic.".into());
            }
        }
    }

    if two_sentence_mode && contains_any_marker(&s1, &spec.step2.strong_markers) {
        score -= 3.0;
        notes.push("Step connectors are slightly mixed, but still understandable.".into());
    }
    if two_sentence_mode && contains_any_marker(&s2, &spec.step1.strong_markers) {
        score -= 3.0;
        notes.push("Step connectors are slightly mixed, but still understandable.".into());
    }

    let seed_len = spec.seed.chars().count();
    let answer_len = answer.chars().count();
    if answer_len >= seed_len.saturating_add(6) {
        score += 6.0;
        notes.push("Creative expansion is rewarded.".into());
    } else if answer_len >= seed_len.saturating_add(2) {
        score += 3.0;
    }
    ...
    let explanation = if notes.is_empty() {
        "Pass: intent is clear and the response follows the challenge direction.".to_string()
    } else if correct {
        format!("Pass: {}", notes.join(" "))
    } else {
        format!(
            "Close: {} Add a clearer relation marker from step 1 ({}) and step 2 ({}).",
            notes.join(" "),
            spec.step1.markers_zh,
            spec.step2.markers_zh
        )
    };
...
const PATTERNS_CAUSE: &[PatternDef] = &[
    pat!(
        "zh_pat__cause__yinwei_suoyi__pair__l1",
        1,
        "PAIR",
        "因为{A}，所以{B}",
        "因为…所以…",
    ...
    pat!(
        "zh_pat__cause__youyu_yinci__pair__l2",
        2,
        "PAIR",
        "由于{A}，因此{B}",
        "由于…因此…",
    ...
    pat!(
        "zh_pat__cause__jiran_jiu__pair__l2",
        2,
        "PAIR",
        "既然{A}，就{B}",
        "既然…就…",
    ...
    pat!(
        "zh_pat__cause__yinwei_only__single__l1",
        1,
        "SINGLE",
        "因为{A}，{B}",
        "因为…",
    ...
    pat!(
        "zh_pat__cause__youyu_only__single__l2",
        2,
        "SINGLE",
        "由于{A}，{B}",
        "由于…",
    ...
    pat!(
        "zh_pat__cause__zhengyinwei__single__l3",
        3,
        "SINGLE",
        "正因为{A}，{B}",
        "正因为…",
    ...
    pat!(
        "zh_pat__cause__b_shiyinwei_a__single__l2",
        2,
        "SINGLE",
        "{B}，是因为{A}",
        "…是因为…",
    ...
    pat!(
        "zh_pat__cause__zhisuoyi_shiyinwei__pair__l2",
        2,
        "PAIR",
        "之所以{B}，是因为{A}",
        "之所以…是因为…",
    ...
    pat!(
        "zh_pat__cause__yuanyin_zaiyu__single__l3",
        3,
        "SINGLE",
        "{B}的原因在于{A}",
        "…的原因在于…",
    ...
    pat!(
        "zh_pat__cause__daozhi__single__l2",
        2,
        "SINGLE",
        "{A}，导致{B}",
        "导致…",
    ...
    pat!(
        "zh_pat__cause__shide__single__l2",
        2,
        "SINGLE",
        "{A}，使得{B}",
        "使得…",
...
const PATTERNS_RESULT: &[PatternDef] = &[
    pat!(
        "zh_pat__result__suoyi__single__l1",
        1,
        "SINGLE",
        "{A}，所以{B}",
        "所以…",
    ...
    pat!(
        "zh_pat__result__yinci__single__l1",
        1,
        "SINGLE",
        "{A}，因此{B}",
        "因此…",
    ...
    pat!(
        "zh_pat__result__yiner__single__l2",
        2,
        "SINGLE",
        "{A}，因而{B}",
        "因而…",
    ...
    pat!(
        "zh_pat__result__yushi__single__l1",
        1,
        "SINGLE",
        "{A}，于是{B}",
        "于是…",
    ...
    pat!(
        "zh_pat__result__jieguo__single__l1",
        1,
        "SINGLE",
        "{A}，结果{B}",
        "结果…",
    ...
    pat!(
        "zh_pat__result__jieguo_shi__single__l2",
        2,
        "SINGLE",
        "{A}，结果是{B}",
        "结果是…",
    ...
    pat!(
        "zh_pat__result__conger__single__l3",
        3,
        "SINGLE",
        "{A}，从而{B}",
        "从而…",
    ...
    pat!(
        "zh_pat__result__jin_er__single__l3",
        3,
        "SINGLE",
        "{A}，进而{B}",
        "进而…",
    ...
    pat!(
        "zh_pat__result__yizhiyu__single__l3",
        3,
        "SINGLE",
        "{A}，以至于{B}",
        "以至于…",
...
const PATTERNS_CONDITION: &[PatternDef] = &[
    pat!(
        "zh_pat__cond__ruguo_jiu__pair__l1",
        1,
        "PAIR",
        "如果{A}，就{B}",
        "如果…就…",
    ...
    pat!(
        "zh_pat__cond__yaoshi_jiu__pair__l1",
        1,
        "PAIR",
        "要是{A}，就{B}",
        "要是…就…",
    ...
    pat!(
        "zh_pat__cond__jiaru_jiu__pair__l2",
        2,
        "PAIR",
        "假如{A}，就{B}",
        "假如…就…",
    ...
    pat!(
        "zh_pat__cond__zhiyao_jiu__pair__l1",
        1,
        "PAIR",
        "只要{A}，就{B}",
        "只要…就…",
    ...
    pat!(
        "zh_pat__cond__zhiyou_cai__pair__l2",
        2,
        "PAIR",
        "只有{A}，才{B}",
        "只有…才…",
    ...
    pat!(
        "zh_pat__cond__chufei_fouze__pair__l2",
        2,
        "PAIR",
        "除非{A}，否则{B}",
        "除非…否则…",
    ...
    pat!(
        "zh_pat__cond__a_dehua_b__single__l2",
        2,
        "SINGLE",
        "{A}的话，{B}",
        "…的话…",
    ...
    pat!(
        "zh_pat__cond__fouze__single__l2",
        2,
        "SINGLE",
        "{A}，否则{B}",
        "否则…",
    ...
    pat!(
        "zh_pat__cond__qingkuangxia__single__l3",
        3,
        "SINGLE",
        "在{A}的情况下，{B}",
        "在…的情况下…",
...
const PATTERNS_CONTRAST: &[PatternDef] = &[
    pat!(
        "zh_pat__contrast__suiran_danshi__pair__l1",
        1,
        "PAIR",
        "虽然{A}，但是{B}",
        "虽然…但是…",
    ...
    pat!(
        "zh_pat__contrast__suiran_dan__pair__l2",
        2,
        "PAIR",
        "虽然{A}，但{B}",
        "虽然…但…",
    ...
    pat!(
        "zh_pat__contrast__jinguan_dan__pair__l2",
        2,
        "PAIR",
        "尽管{A}，但{B}",
        "尽管…但…",
    ...
    pat!(
        "zh_pat__contrast__jinguan_rengran__pair__l3",
        3,
        "PAIR",
        "尽管{A}，仍然{B}",
        "尽管…仍然…",
    ...
    pat!(
        "zh_pat__contrast__a_buguo_b__single__l1",
        1,
        "SINGLE",
        "{A}，不过{B}",
        "不过…",
    ...
    pat!(
        "zh_pat__contrast__a_keshi_b__single__l1",
        1,
        "SINGLE",
        "{A}，可是{B}",
        "可是…",
    ...
    pat!(
        "zh_pat__contrast__a_ran'er_b__single__l2",
        2,
        "SINGLE",
        "{A}，然而{B}",
        "然而…",
    ...
    pat!(
        "zh_pat__contrast__a_que_b__single__l2",
        2,
        "SINGLE",
        "{A}，却{B}",
        "却…",
    ...
    pat!(
        "zh_pat__contrast__a_faner_b__single__l3",
        3,
        "SINGLE",
        "{A}，反而{B}",
        "反而…",
    ...
    pat!(
        "zh_pat__contrast__biaomianshang_qishi__pair__l3",
        3,
        "PAIR",
        "表面上{A}，其实{B}",
        "表面上…其实…",
    ...
    pat!(
        "zh_pat__contrast__yifangmian_lingyifangmian__pair__l3",
        3,
        "PAIR",
        "一方面{A}，另一方面{B}",
        "一方面…另一方面…",
...
const PATTERNS_TIME: &[PatternDef] = &[
    pat!(
        "zh_pat__time__dang_shi__single__l1",
        1,
        "SINGLE",
        "当{A}的时候，{B}",
        "当…的时候…",
    ...
    pat!(
        "zh_pat__time__zai_shi__single__l2",
        2,
        "SINGLE",
        "在{A}的时候，{B}",
        "在…的时候…",
    ...
    pat!(
        "zh_pat__time__a_yihou_b__single__l1",
        1,
        "SINGLE",
        "{A}以后，{B}",
        "…以后…",
    ...
    pat!(
        "zh_pat__time__a_zhihou_b__single__l1",
        1,
        "SINGLE",
        "{A}之后，{B}",
        "…之后…",
    ...
    pat!(
        "zh_pat__time__a_zhiqian_b__single__l1",
        1,
        "SINGLE",
        "{A}之前，{B}",
        "…之前…",
    ...
    pat!(
        "zh_pat__time__cong_kaishi__single__l2",
        2,
        "SINGLE",
        "从{A}开始，{B}",
        "从…开始…",
    ...
    pat!(
        "zh_pat__time__zicong_yihou__single__l3",
        3,
        "SINGLE",
        "自从{A}以后，{B}",
        "自从…以后…",
    ...
    pat!(
        "zh_pat__time__suizhe__single__l3",
        3,
        "SINGLE",
        "随着{A}，{B}",
        "随着…",
    ...
    pat!(
        "zh_pat__time__meidang__single__l3",
        3,
        "SINGLE",
        "每当{A}，{B}",
        "每当…",
...
const PATTERNS_PURPOSE: &[PatternDef] = &[
    pat!(
        "zh_pat__purpose__weile__single__l1",
        1,
        "SINGLE",
        "为了{B}，{A}",
        "为了…",
    ...
    pat!(
        "zh_pat__purpose__a_weile_b__single__l1",
        1,
        "SINGLE",
        "{A}，为了{B}",
        "…为了…",
    ...
    pat!(
        "zh_pat__purpose__yibian__single__l2",
        2,
        "SINGLE",
        "{A}，以便{B}",
        "以便…",
    ...
    pat!(
        "zh_pat__purpose__haorang__single__l2",
        2,
        "SINGLE",
        "{A}，好让{B}",
        "好让…",
    ...
    pat!(
        "zh_pat__purpose__weideshi__single__l2",
        2,
        "SINGLE",
        "{A}，为的是{B}",
        "为的是…",
    ...
    pat!(
        "zh_pat__purpose__mian_de__single__l3",
        3,
        "SINGLE",
        "{A}，免得{B}",
        "免得…",
    ...
    pat!(
        "zh_pat__purpose__yimian__single__l3",
        3,
        "SINGLE",
        "{A}，以免{B}",
        "以免…",
    ...
    pat!(
        "zh_pat__purpose__weib_qijian__single__l3",
        3,
        "SINGLE",
        "为{B}起见，{A}",
        "为…起见…",
...
const PATTERNS_ADDITION: &[PatternDef] = &[
    pat!(
        "zh_pat__add__budan_erqie__pair__l2",
        2,
        "PAIR",
        "不但{A}，而且{B}",
        "不但…而且…",
    ...
    pat!(
        "zh_pat__add__buji_hai__pair__l1",
        1,
        "PAIR",
        "不仅{A}，还{B}",
        "不仅…还…",
    ...
    pat!(
        "zh_pat__add__a_erqie_b__single__l1",
        1,
        "SINGLE",
        "{A}，而且{B}",
        "而且…",
    ...
    pat!(
        "zh_pat__add__a_bingqie_b__single__l2",
        2,
        "SINGLE",
        "{A}，并且{B}",
        "并且…",
    ...
    pat!(
        "zh_pat__add__a_tongshi_b__single__l2",
        2,
        "SINGLE",
        "{A}，同时{B}",
        "同时…",
    ...
    pat!(
        "zh_pat__add__a_yebing_b__single__l1",
        1,
        "SINGLE",
        "{A}，也{B}",
        "也…",
    ...
    pat!(
        "zh_pat__add__chule_hai__pair__l3",
        3,
        "PAIR",
        "除了{A}以外，还{B}",
        "除了…以外，还…",
...
const PATTERNS_CHOICE: &[PatternDef] = &[
    pat!(
        "zh_pat__choice__yaome_yaome__pair__l1",
        1,
        "PAIR",
        "要么{A}，要么{B}",
        "要么…要么…",
    ...
    pat!(
        "zh_pat__choice__huozhe_huozhe__pair__l2",
        2,
        "PAIR",
        "或者{A}，或者{B}",
        "或者…或者…",
    ...
    pat!(
        "zh_pat__choice__bushi_jiushi__pair__l2",
        2,
        "PAIR",
        "不是{A}，就是{B}",
        "不是…就是…",
    ...
    pat!(
        "zh_pat__choice__a_huozhe_b__single__l1",
        1,
        "SINGLE",
        "{A}，或者{B}",
        "或者…",
    ...
    pat!(
        "zh_pat__choice__yuqi_buru__pair__l3",
        3,
        "PAIR",
        "与其{A}，不如{B}",
        "与其…不如…",
    ...
    pat!(
        "zh_pat__choice__ningke_yebu__pair__l3",
        3,
        "PAIR",
        "宁可{A}，也不{B}",
        "宁可…也不…",
...
const CHAIN_PATTERNS: &[ChainPatternDef] = &[
    ChainPatternDef {
        id: "zh_chain__cause_to_result__v1",
        step1: REL_CAUSE,
        step2: REL_RESULT,
        scene_schema: "reason_outcome_followup",
    },
    ChainPatternDef {
        id: "zh_chain__condition_to_result__v1",
        step1: REL_CONDITION,
        step2: REL_RESULT,
        scene_schema: "condition_outcome_followup",
    },
    ChainPatternDef {
        id: "zh_chain__time_to_result__v1",
        step1: REL_TIME,
        step2: REL_RESULT,
        scene_schema: "time_event_outcome",
    },
    ChainPatternDef {
        id: "zh_chain__contrast_to_result__v1",
        step1: REL_CONTRAST,
        step2: REL_RESULT,
        scene_schema: "expectation_actual_consequence",
    },
    ChainPatternDef {
        id: "zh_chain__choice_to_condition__v1",
        step1: REL_CHOICE,
        step2: REL_CONDITION,
        scene_schema: "optionA_optionB_then_rule",
    },
    ChainPatternDef {
        id: "zh_chain__addition_to_result__v1",
        step1: REL_ADDITION,
        step2: REL_RESULT,
        scene_schema: "fact1_fact2_inference",
    },
    ChainPatternDef {
        id: "zh_chain__purpose_to_result__v1",
        step1: REL_PURPOSE,
        step2: REL_RESULT,
        scene_schema: "action_goal_effect",
    },
    ChainPatternDef {
        id: "zh_chain__time_to_contrast__v1",
        step1: REL_TIME,
        step2: REL_CONTRAST,
        scene_schema: "time_then_now_contrast",
    },
    ChainPatternDef {
        id: "zh_chain__condition_to_contrast__v1",
        step1: REL_CONDITION,
        step2: REL_CONTRAST,
        scene_schema: "condition_expected_surprise",
    },
];

const SCENES: &[SceneDef] = &[
    scene!(
        "zh_scene__study_sleep__v1",
        "reason_outcome_followup",
        "我没睡够",
        "我还是把笔记整理完了",
        "第二天上课更轻松了"
    ...
    scene!(
        "zh_scene__daily_no_umbrella__v1",
        "reason_outcome_followup",
        "我忘带伞",
        "衣服被淋湿了",
        "回家后我立刻换了衣服"
    ...
    scene!(
        "zh_scene__phone_low_battery__v1",
        "reason_outcome_followup",
        "手机电量太低",
        "导航一直提醒省电模式",
        "我找了个地方先充电"
    ...
    scene!(
        "zh_scene__finish_experiment__v1",
        "time_event_outcome",
        "实验做完了",
        "我马上记录数据",
        "后面分析更顺利"
    ...
    scene!(
        "zh_scene__project_deadline__v1",
        "time_event_outcome",
        "截止日期到了",
        "我把最后一版提交上去",
        "我终于松了口气"
    ...
    scene!(
        "zh_scene__expect_easy_but_hard__v1",
        "expectation_actual_consequence",
        "我以为今天会很顺利",
        "事情却特别多",
        "我忙到很晚才结束"
    ),
    scene!(
        "zh_scene__expect_fast_but_slow__v1",
        "expectation_actual_consequence",
        "我以为十分钟就能搞定",
        "过程却拖了很久",
        "我后面的安排被迫改了"
    ...
    scene!(
        "zh_scene__expect_quiet_but_noisy__v1",
        "expectation_actual_consequence",
        "我以为咖啡店会很安静",
        "里面却很吵",
        "我只好换个地方"
    ...
    scene!(
        "zh_scene__expect_cheaper_but_expensive__v1",
        "expectation_actual_consequence",
        "我以为修理不会太贵",
        "费用却超出预算",
        "我只能先做最必要的部分"
    ...
    scene!(
        "zh_scene__expect_ready_but_missing__v1",
        "expectation_actual_consequence",
        "我以为资料都准备好了",
        "关键文件却找不到",
        "我只好重新整理一遍"
    ...
    scene!(
        "zh_scene__expect_good_weather_but_rain__v1",
        "expectation_actual_consequence",
        "我以为今天不会下雨",
        "外面却突然变天",
        "我被淋得有点狼狈"
    ...
    scene!(
        "zh_scene__option_cook_or_takeout__v1",
        "optionA_optionB_then_rule",
        "我可以自己做饭",
        "我也可以点外卖",
        "如果我点外卖，就要多等一会儿"
    ),
    scene!(
        "zh_scene__option_walk_or_bus__v1",
        "optionA_optionB_then_rule",
        "我可以走路过去",
        "我也可以坐公交",
        "如果我坐公交，就得看发车时间"
    ...
    scene!(
        "zh_scene__option_train_or_taxi__v1",
        "optionA_optionB_then_rule",
        "我可以坐地铁",
        "我也可以打车",
        "如果我打车，就会多花一些钱"
    ...
    scene!(
        "zh_scene__option_now_or_later__v1",
        "optionA_optionB_then_rule",
        "我可以现在就开始",
        "我也可以拖到明天",
        "如果我拖到明天，就会更赶"
    ...
    scene!(
        "zh_scene__two_facts_save_time__v1",
        "fact1_fact2_inference",
        "我把路线提前查好了",
        "出门前也准备齐东西",
        "路上省了不少时间"
    ...
    scene!(
        "zh_scene__two_facts_more_focus__v1",
        "fact1_fact2_inference",
        "我把手机调成静音",
        "我还关掉了消息提醒",
        "我更能专心做事"
    ...
    scene!(
        "zh_scene__prepare_questions__v1",
        "action_goal_effect",
        "我提前准备了问题清单",
        "沟通更高效",
        "会议时间缩短了"
    ...
    scene!(
        "zh_scene__then_now_study__v1",
        "time_then_now_contrast",
        "以前我复习很随意",
        "我经常记不住重点",
        "现在我却更有方法了"
    ),
    scene!(
        "zh_scene__then_now_sleep__v1",
        "time_then_now_contrast",
        "以前我总是熬夜",
        "我白天很没精神",
        "现在我反而睡得更规律"
    ...
    scene!(
        "zh_scene__then_now_workflow__v1",
        "time_then_now_contrast",
        "刚开始我不太会用这个工具",
        "我做事很慢",
        "后来我却越来越熟练"
    ...
    scene!(
        "zh_scene__prepared_but_mistake__v1",
        "condition_expected_surprise",
        "我提前准备了",
        "事情本来应该很顺利",
        "结果却还是出了差错"
    ),
    scene!(
        "zh_scene__leave_early_but_late__v1",
        "condition_expected_surprise",
        "我出门很早",
        "我本来应该不会迟到",
        "路上却偏偏堵得厉害"
    ...
    scene!(
        "zh_scene__practice_but_nervous__v1",
        "condition_expected_surprise",
        "我练习了很多次",
        "我本来应该很自信",
        "上台却还是有点紧张"
