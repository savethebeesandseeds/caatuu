...
const gameRoot = new URL("apps/games/caatuu-game/", repoRoot);
...
const [
  ...
  await Promise.all([
    ...
    readFile(new URL("scripts/caatuu_game.gd", gameRoot), "utf8"),
...
test("Caatuu Game has one browser-first Godot project using the Web-compatible renderer", () => {
...
test("Caatuu Game renders the world and HUD directly at native root resolution", () => {
  assert.match(mainScene, /node name="CaatuuGame" type="Node3D"/);
  assert.doesNotMatch(mainScene, /SubViewport|TextureRect|caatuu_game_screen/);
...
test("the Godot exporter is pinned, isolated, non-root, and verifies its shared motion source", () => {
  ...
  assert.match(compose, /caatuu-game-godot-provision:/);
  assert.match(compose, /caatuu-game-godot-export:[\s\S]*?network_mode: none/);
  assert.match(compose, /caatuu-game-godot-export:[\s\S]*?read_only: true/);
  ...
  assert.match(compose, /caatuu-godot-web-toolchain-4-7-1:\/toolchain:ro/);
  assert.match(
    compose,
...
test("Memory Grove v6 streams a reusable terrain tile map and navigation into the Web export", () => {
  const sceneryMarker =
    "CAATUU_GAME_SCENERY_SMOKE_OK layout=memory-grove-v6 terrain=chunk-stream chunks=49/144 tiles=441/1296 map_tiles=144 atlas_tiles=48 used_tile_types=32 navigation=true prop_placements=18 collision_objects=18 shadows=0";
  const catalog = JSON.parse(activeSceneryCatalog);
  const layout = JSON.parse(activeSceneryLayout);
  ...
  assert.equal(
    ...
    false,
    "Caatuu Game must consume the canonical scenery catalog instead of keeping a game-local copy"
  );
  assert.match(gameScript, /WorldSceneryBuilder := preload/);
  assert.match(gameScript, /scenery_builder\.build\(self\)/);
  assert.match(sceneryScript, /class_name CaatuuGameScenery/);
  ...
  assert.match(sceneryScript, /CATALOG_ID := "caatuu-game-style-v1"/);
  ...
  assert.equal(layout.catalog_id, "caatuu-game-style-v1");
  ...
  assert.match(exporter, /LICENSES\/Caatuu-Game-Style-Provenance\.md/);
  ...
  for (const document of [caatuuGameReadme, thirdPartyNotices]) {
    assert.match(document, /metadata\/catalog\.json/);
    assert.match(document, /metadata\/world\.json/);
  }
  assert.match(caatuuGameReadme, /local-preview\s+rights/);
  assert.match(thirdPartyNotices, /local-preview-only/);
  assert.match(caatuuGameReadme, /apps\/launcher\/static\/assets\/scenery\//);
  assert.match(caatuuGameReadme, /consumer[^.]+no canonical scenery copy/is);
  assert.match(caatuuGameReadme, /Physical storage is intentionally unversioned and shallow/i);
  assert.match(
    caatuuGameReadme,
    /exporter\s+stages\s+the\s+minimum active, hash-verified subset/
  );
  assert.match(caatuuGameReadme, /copy is disposable build output/);
  assert.match(
    caatuuGameReadme,
...
test("the standalone game has no parent-window embedding protocol", () => {
  assert.doesNotMatch(gameScript, /JavaScriptBridge|window\.parent|postMessage/);
});
