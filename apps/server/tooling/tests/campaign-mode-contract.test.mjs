...
const sharedStaticRoot = new URL("apps/language-runtime/static/", repoRoot);
...
const [
  ...
  readFile(new URL("styles/caatuu-workspace.css", sharedStaticRoot), "utf8"),
  ...
  readFile(new URL("apps/language-runtime/static/source/product-word-world.mjs", repoRoot), "utf8"),
...
test("Campaign Mode is a prominent mixed-play route with its requested artwork", () => {
  ...
  assert.doesNotMatch(appBootstrap, /source\/features\/campaign\/campaign\.css/);
...
test("the shell validates shared and iframe success and shows a robot before switching planets", () => {
  assert.match(app, /function handleCampaignGameMessage\(event\)[\s\S]*?event\.origin !== window\.location\.origin/);
  assert.match(app, /message\?\.source !== "caatuu-game" \|\| message\.type !== "round-success"/);
  assert.match(app, /gameId === "word-net" && event\.source === window[\s\S]*?completeCampaignRound\(gameId, window\)/);
  assert.match(app, /const frame = campaignFrame\(gameId\)[\s\S]*?event\.source !== frame\.contentWindow/);
  ...
  assert.match(wordWorld, /lifecycleOptions\.onRoundSuccess\?\.\(payload\)[\s\S]*?window\.postMessage\?\.\(payload, window\.location\.origin\)/);
