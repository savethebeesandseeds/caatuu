...
pub async fn do_translate_with_mode(state: &AppState, text: &str, fast_only: bool) -> String {
    if let Some(oa) = &state.openai {
        ...
        match out {
            Ok(t) => return t,
            Err(e) => {
                tracing::error!(target: "caatuu_runtime", error = %e, "OpenAI translate failed; using stub fallback.")
            }
        }
    }
    translate_stub(text)
...
pub async fn do_grammar_with_mode(state: &AppState, text: &str, fast_only: bool) -> String {
    if let Some(oa) = &state.openai {
        ...
        match out {
            Ok(t) => return t,
            Err(e) => {
                tracing::error!(target: "caatuu_runtime", error = %e, "OpenAI grammar_correct failed; using stub fallback.")
            }
        }
    }
    grammar_stub(text)
...
pub async fn do_agent_reply_with_mode(
    ...
    if let Some(oa) = &state.openai {
        ...
        match out {
            Ok(t) => {
                debug!(target: "caatuu_runtime", %challenge_id, has_context = ctx.is_some(), "Agent reply via OpenAI.");
                t
            }
            Err(e) => {
                tracing::error!(target: "caatuu_runtime", %challenge_id, error = %e, "Agent reply failed; using stub.");
                agent_reply_stub(question)
            }
        }
    } else {
        debug!(target: "caatuu_runtime", %challenge_id, "Agent reply via stub.");
        agent_reply_stub(question)
    }
}
