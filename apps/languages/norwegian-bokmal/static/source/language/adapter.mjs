...
function segmentNorwegian(value) {
...
function prepareSpeech(value) {
  const text = normalizeText(value);
  if (!text) throw new TypeError("Norwegian Bokmål speech requires non-empty text.");
  if (text.length > 1_000) throw new RangeError("Norwegian Bokmål speech supports up to 1,000 characters.");
  return text;
}
...
function presentDictionaryEntry(record, context = {}) {
  if (!isRecord(record)) {
    throw new TypeError("Norwegian Bokmål dictionary presentation requires an object record.");
  }
...
export const norwegianBokmalLanguageAdapter = defineLanguageAdapter({
  ...
  dictionary: {
    lookupKey(value) {
      return normalizeWord(value).toLocaleLowerCase("nb-NO");
    },
...
export default norwegianBokmalLanguageAdapter;
