...
test("the backpack progression hub has a distinct reward-focused surface", () => {
  ...
  const settingsLanguage = ruleWithSelector(chromeCss, ".settings-language-pill");
  assert.equal(settingsLanguage.declarations.get("border"), "0");
  assert.equal(settingsLanguage.declarations.get("background"), "transparent");
  const settingsLanguageCode = ruleWithSelector(chromeCss, ".settings-language-pill .language-code");
  assert.equal(settingsLanguageCode.declarations.get("display"), "none");
