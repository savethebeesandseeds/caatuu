...
PID_FILE="$PROJECT_ROOT/.caatuu-runtime.pid"
...
stop_backend() {
  ...
  # Stop runtime processes in case the PID file is stale or missing.
  local orphan_pids
  orphan_pids="$(
    {
      pgrep -x caatuu-runtime || true
      pgrep -x caatuu || true
    } | awk 'NF' | sort -u
  )"
  if [[ -n "$orphan_pids" ]]; then
    echo "Stopping runtime orphan PID(s) (caatuu-runtime/caatuu): $orphan_pids"
...
echo "  OPENAI_WRITING_MODEL=${OPENAI_WRITING_MODEL:-}"
