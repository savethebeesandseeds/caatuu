...
Caatuu Game is Caatuu's browser-first Godot integration spike. The authored
...
Caatuu Game is a consumer and keeps no canonical scenery copy in its game
...
records the logical `caatuu-game-style-v1` catalog and `memory-grove-v6` world
as the active pair.
...
window override, and `expand` stretch behavior. The standalone canvas preserves
a 16:9 stage on wider screens and uses the available viewport in phone portrait.
The world surface accepts the same click or tap gesture on desktop and browser
touch; only reset and appearance remain buttons.
...
docker compose --profile game-tools-provision build caatuu-game-godot-provision
docker compose --profile game-tools-provision run --rm caatuu-game-godot-provision
docker compose --profile game-tools build caatuu-game-godot-export
docker compose --profile game-tools run --rm caatuu-game-godot-export
...
`artifacts/games/caatuu-game/web/godot-v1/`. It is intentionally ignored
...
directory at the neutral `/games/caatuu-game/godot-v1/` route. The stable
`/games/caatuu-game/` URL redirects to that versioned directory. There is no
`/cz/games/**` compatibility alias and the Caatuu application service worker
does not own the standalone game route.

[`game.json`](game.json) is the machine-readable authority for the game ID,
engine, source, generated delivery path, platforms, standalone browser mode,
dependencies, notices, and preview-only release status. The game deliberately
has no language adapter or Android delivery contract.
