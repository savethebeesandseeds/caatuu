...
pub async fn ws_upgrade(
    ...
    info!(target: "caatuu_runtime", "WebSocket upgrade requested");
    ws.on_upgrade(move |socket| handle_ws(socket, state))
}

#[instrument(level = "info", skip(socket, state))]
async fn handle_ws(mut socket: WebSocket, state: Arc<AppState>) {
    info!(target: "caatuu_runtime", "WebSocket connected");
        ...
        match msg {
            Message::Text(txt) => {
                ...
                let reply_msg = match serde_json::from_str::<ClientWsMessage>(&txt) {
                    Ok(incoming) => {
                        ...
                        debug!(target = "caatuu_runtime", %summary, "WS received");
          ...
          serde_json::json!({ "type": "error", "message": format!("Serialization error: {}", e) }).to_string()
                ...
                if let Err(e) = socket.send(Message::Text(out)).await {
                    error!(target: "caatuu_runtime", error = %e, "WS send error");
                    break;
                }
            }
    ...
    info!(target: "caatuu_runtime", "WebSocket disconnected");
}
...
async fn handle_client_ws(msg: ClientWsMessage, state: &AppState) -> ServerWsMessage {
    match msg {
        ...
        ClientWsMessage::AgentReset => {
            tracing::info!(target: "caatuu_runtime", "WS agent_reset noop acknowledged");
