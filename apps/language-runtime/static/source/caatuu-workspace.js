...
// Course capabilities select providers; they do not select another shell controller.
const courseCapabilities = course.capabilities && typeof course.capabilities === "object"
  ? course.capabilities
  : {};
const courseRouteBase = new URL(
...
function courseGameAvailable(gameId) {
  const policyResult = window.CaatuuShellPolicy?.gameAvailable?.(course, gameId);
  if (typeof policyResult === "boolean") return policyResult;
  const routeByGame = {
    "verb-lab": "verbNebula",
    "word-net": "wordWorld",
  ...
  return Boolean(route && course.games?.includes?.(gameId) && course.routes?.[route]);
}

function courseUsesModels() {
...
function openSettingsPanel() {
  ...
  syncThemeControls();
  if (courseUsesModels()) syncGenerationSettingsUi();
...
const verbStorageKey = course.storage.verbMemory || `${course.storage.namespace}.verb-memory.v3`;
...
async function refreshVerbAudioVoiceControls() {
  ...
  result.voices.forEach((voice) => {
    ...
    option.textContent = `${voice.name} (${voice.locale || verbTargetLabel} · ${voice.service})`;
...
function speakVerbCzechOnTap(verbId) {
  ...
  void window.CaatuuChrome.speakText(target).catch((error) => {
    console.warn(`Could not speak the selected ${verbTargetLabel} verb.`, error);
  });
}
...
function recordVerbSemanticAttempt(pair, {
  ...
  void semanticLearning.recordAttempt({
    ...
    signals: [
      {
        conceptId: `${targetLanguage.id || "target"}.verb.${pair.id}.meaning`,
        statementRevision: "1",
        kind: "meaning",
        locale: "en",
      ...
      {
        conceptId: `${targetLanguage.id || "target"}.verb.${pair.id}.meaning-match`,
        statementRevision: "1",
        kind: "skill",
        locale: "en",
  ...
  if (correct) {
    ...
    if (!state.verbGuidedMode) {
      ...
      recordVerbSemanticAttempt(pair, {
        correct: true,
        chosenSource: pair?.source ?? pair?.eng ?? "",
        roundComplete
      });
    }
    saveVerbMemory();
...
function bindVerbNebulaControls() {
  ...
  $("#verbAudioVoice")?.addEventListener("change", async (event) => {
    ...
    select.disabled = true;
    await window.CaatuuChrome?.stopSpeech?.();
...
function normalizeView(view) {
  ...
  if ((view === "guide" || view === "dictionary") && courseHasCapability("dictionary")) return "dictionary";
...
function ensureWordNetLoaded() {
  const host = window.CaatuuWordWorldHost;
  const status = document.getElementById("wordNetEmbeddedStatus");
  if (!host || typeof host.ensureLoaded !== "function") {
    if (status) {
  ...
  return Promise.resolve(host.ensureLoaded()).then(() => true).catch((error) => {
...
function campaignFrame(gameId) {
  if (gameId === "word-net") return document.getElementById("wordNetEmbeddedStage");
  const config = embeddedGameTabs[gameId];
...
async function waitForCampaignGameReady(gameId, transitionId) {
  ...
  while (gameId === "word-net"
    ? window.CaatuuWordWorldHost?.ready?.() !== true
    : frame.dataset.ready !== "true") {
...
function render() {
  ...
  if (courseGameAvailable("verb-lab")) renderVerbNebula();
}
...
function renderDataError(error) {
  ...
  const list = $("#dictionaryList");
  if (list) list.innerHTML = `<p class="empty-state">Could not load course data. Open the app from the local server and reload.</p>`;
}
...
function bindUi() {
  ...
  document.addEventListener("click", (event) => {
    ...
    if (trainTab) {
      ...
      if (selectedTab !== "galaxy" && !courseGameAvailable(selectedTab)) return;
      event.preventDefault();
  ...
  if (courseGameAvailable("verb-lab")) bindVerbNebulaControls();
}
...
window.CaatuuWorkspaceShell = Object.freeze({
  ...
  state() {
    return Object.freeze({
      ...
      campaignTransitioning: state.campaignTransitioning
    });
  }
});
