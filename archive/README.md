# Archive

This directory preserves superseded product implementations that remain useful
for history or future reference. Archived code is not part of the active
runtime contract and should not be used as a source of current setup guidance.

- [`caatuu-chinese`](caatuu-chinese/) preserves the earlier Chinese language
  application while the active reference course is Czech.

Large local checkouts, build outputs, dependency caches, signing material, and
nested Git repositories are not archival source and are intentionally ignored.
When they must be retained on one workstation, they live under ignored
`archive/local/` with descriptive names rather than appearing as active archive
entries.
